...
export async function POST(request: Request) {
  ...
  response = NextResponse.json({ error: 'Nenhum lançamento fornecido para importação' }, { status: 400 });
  } else {
  ...
  response = NextResponse.json({ inseridos: normalizados.length });
  }
  } catch (error) {
  console.error('Erro ao importar lançamentos:', error);
  response = NextResponse.json({ error: 'Erro ao importar lançamentos' }, { status: 500 });
  }
  return response;
}
import { NextResponse } from 'next/server'
